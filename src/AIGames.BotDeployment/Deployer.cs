...
namespace AIGames.BotDeployment
{
	...
	public class Deployer
	{
		...
		public static void CopyCSharpFiles(DirectoryInfo source, DirectoryInfo target, bool debug = false)
		{
			...
			var libfiles = CollectCSharpFiles(GetCoreDir(), debug).ToList();
		...
		public static IEnumerable<FileInfo> CollectCSharpFiles(DirectoryInfo dir, bool debug = false)
		{
			...
			foreach (var file in dir.GetFiles("*.cs"))
			{
				...
				else if (!file.Name.EndsWith(".Debug.cs", StringComparison.InvariantCultureIgnoreCase) && 
					!file.Name.Equals("AssemblyInfo.cs", StringComparison.InvariantCultureIgnoreCase))
				{
					yield return file;
				}
			}
		...
		/// <summary>Gets the configured core directory.</summary>
		/// <remarks>
		/// The core directory is the root directory with the AIGames.HeadsUpOmaha sources.
		/// </remarks>
		public static DirectoryInfo GetCoreDir()
		{
			var dir = ConfigurationManager.AppSettings["Core.Dir"];
		...
		{
			using (var reader = new StreamReader(typeof(Deployer).Assembly.GetManifestResourceStream("AIGames.BotDeployment.README.md")))
			{
